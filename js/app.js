 ...
 */
    ...
    (() => {
        ...
        da.init();
        const script_form = document.querySelector(".form__form");
        const formLines = script_form.querySelectorAll(".form__line");
        const phoneInput = script_form.querySelector(".form__line.phone");
        const countryList = phoneInput.querySelector(".form__list");
        const counties = countryList.querySelectorAll(".form__list-item");
        const inputs = script_form.querySelectorAll(".form__input");
        const chnageCountryButton = script_form.querySelector(".form__change-icon");
        let phoneCountry = "";
        for (const formLine of formLines) {
            const formLabel = formLine.querySelector(".form__label");
            const formInput = formLine.querySelector(".form__input");
            formLine.addEventListener("click", (() => {
                formLabel.classList.add("active");
                if (!formLine.classList.contains("phone")) formInput.focus();
            }));
            formInput.addEventListener("blur", (() => {
                if (!formInput.value) formLabel.classList.remove("active");
            }));
            formInput.addEventListener("input", (() => {
                if (formLine.classList.contains("hasError") && formInput.value.trim()) {
                    const error = formLine.querySelector(".form__error");
                    formLine.classList.remove("hasError");
                    error.innerHTML = "";
                }
            }));
        }
        phoneInput.addEventListener("click", (() => {
            if (!phoneInput.classList.contains("changed")) countryList.classList.add("active");
        }));
        chnageCountryButton.addEventListener("click", (() => {
            countryList.classList.add("active");
        }));
        for (const country of counties) country.addEventListener("click", (e => {
            const input = phoneInput.querySelector(".form__input");
            const dataMask = country.getAttribute("data-mask");
            const countryName = country.getAttribute("data-country");
            let maska = new Inputmask(dataMask);
            e.stopPropagation();
            countryList.classList.remove("active");
            phoneInput.classList.add("changed");
            input.removeAttribute("disabled");
            input.setAttribute("placeholder", dataMask);
            input.focus();
            maska.mask(input);
        ...
        document.addEventListener("click", (e => {
            ...
            const its_countryList = target == countryList || countryList.contains(target);
            const its_phoneInput = target == phoneInput || phoneInput.contains(target);
            const countryList_is_active = countryList.classList.contains("active");
            if (!its_countryList && !its_phoneInput && countryList_is_active) countryList.classList.remove("active");
        }));
        script_form.addEventListener("submit", (event => {
            const formData = {};
            let isValid = true;
            for (const formLine of formLines) {
                const input = formLine.querySelector(".form__input");
                const error = formLine.querySelector(".form__error");
                ...
                if (!input.value.trim()) {
                    ...
                    formLine.classList.add("hasError");
                }
                if (input.getAttribute("type") === "email" && input.value.trim() && !emailPattern.test(input.value)) {
                    isValid = false;
                    error.innerHTML = "Не корректный формат электронной почты";
                    formLine.classList.add("hasError");
                }
                if (input.getAttribute("type") === "tel" && input.value && !phonePattern.test(cleanString(input.value))) {
                    isValid = false;
                    error.innerHTML = "Не корректный формат телефона";
                    formLine.classList.add("hasError");
                }
            }
            event.preventDefault();
            inputs.forEach((input => {
                if (input.type = "tel") formData[input.name] = cleanString(input.value); else formData[input.name] = input.value;
            }));
            if (isValid) {
        ...
        function sendMessage(data) {
            const token = "7289976459:AAHy4r5wjrPytsDShaEpxW0dwxaelHBvCeY";
            const chatId = "695310723";
            const message = `Name: ${data.name}\nPhone: ${data.phone}\nEmail: ${data.mail}`;
            fetch(`https://api.telegram.org/bot${token}/sendMessage?chat_id=${chatId}&text=${encodeURIComponent(message)}`).then((response => response.json())).then((data => {
                if (data.ok) alert("Message sent successfully!"); else alert("Error sending message");
            })).catch((error => {
        ...
        function resetForm() {
            for (const formLine of formLines) {
                const formLabel = formLine.querySelector(".form__label");
                const formInput = formLine.querySelector(".form__input");
                let maska = new Inputmask;
                formInput.value = "";
                formLabel.classList.remove("active");
                formLine.classList.remove("changed");
                if (formInput.type === "tel") maska.mask(formInput);
            }
        }
        window["FLS"] = true;
    })();
